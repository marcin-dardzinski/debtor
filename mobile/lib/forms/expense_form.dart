...
class ExpenseFormState extends State<ExpenseForm> {
    ...
    return Form(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          DropdownButtonFormField<User>(items: _getDropdownMenuItems(), value: _currentBorrower, onChanged: (value) => setState(() => _currentBorrower = value), onSaved: (value) => expense.borrower = _currentBorrower),
          DropdownButtonFormField<User>(items: _getDropdownMenuItems(), value: _currentPayer, onChanged: (value) => setState(() => _currentPayer = value), onSaved: (value) => expense.payer = _currentPayer),
  ...
  List<DropdownMenuItem<User>> _getDropdownMenuItems() {
    return widget.availableParticipants.map((x) => DropdownMenuItem(value: x, child: Text(x.name))).toList();
  }
}